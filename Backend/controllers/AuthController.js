...
export const signup = async (request,response,next)=>{
    ...
    try{
        ...
        if(!email  || !password){
            return response.status(400).send("EMAIL AND PASSOWRD IS REQUIRED!");
        }
 ...
 export const updateProfile = async(request,response,next)=>{
    try{
        ...
        if(!firstName || !lastName || !color){
            return response.status(400).send("FirstName LastnName and Color is required.");
        }
...
export const logout = async(request,response,next)=>{
    try{
        ...
        return response.status(200).send("Logout succesful.");
   }catch(error){
       console.log({error});
       return response.status(500).send("Internal Server Error");
   }
