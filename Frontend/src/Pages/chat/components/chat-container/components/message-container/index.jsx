...
function MessageContainer() {
  ...
  const {
    ...
    setSelectedChatMessages,setIsDownloading,setFileDownloadProgress
  } = useAppStore();
    ...
    const getChannelMessages = async()=>{
      try {
        const response = await apiClient.get(
          `${GET_CHANNEL_MESSAGES_ROUTE}/${selectedChatData._id}`,{ withCredentials: true }
        );
        if (response.data.messages) {
          setSelectedChatMessages(response.data.messages);
        }
    ...
    >
        ...
        <div
          className={`${
            message.sender !== selectedChatData._id
              ? "bg-[#8417ff]/5  text-[#00DFC0]/90 border-[#00DFC0]/50"
      ...
      >
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          <span 
          className=" cursor-pointer text-white/80 text-3xl bg-black/20 transition-all
           duration-300 rounded-full p-3 hover:bg-black/50"
  ...
  return (
    <div
      ...
      p-4 px-8 md:w-[65vw] lg:w-[70vw] xl:w-[80vw] sm:w-[full] chat-background`}
    >
      {renderMessages()}
      ...
      {showImage && 
        <div className="fixed z-[1000] top-0 left-0 h-[100vh] w-[100vw] flex items-center justify-center backdrop-blur-lg flex-col">
          ...
          <div className="flex gap-5 fixed top-0 mt-5">
              <button className=" cursor-pointer text-white/80 text-3xl bg-black/20 transition-all
           duration-300 rounded-full p-3 hover:bg-black/50"
           onClick={()=>downloadFile(imageURL)}
           >

             <IoArrowDownCircle/> </button>
             <button className=" cursor-pointer text-white/80 text-3xl bg-black/20 transition-all
           duration-300 rounded-full p-3 hover:bg-black/50"
